...
*   **智能抗波动**: 采用“快速试探 + 稳健重试”的双重超时策略（Fail Fast），既能秒拒死链，又能耐心等待慢网。
*   **交互式跳过**: 下载卡顿时支持按 `Ctrl+C` 立即跳过当前任务，继续执行后续队列，不再因为一条坏链卡死全程序。
*   **原子化数据安全**: 稳健的 CSV 数据库管理，支持目录自动同步，防止记录丢失。
*   **智能索引页面**: 生成支持**实时搜索**、**多维排序**（时间/作者）及**关键字高亮**的单页面索引，完全离线可用。
