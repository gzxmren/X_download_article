...
### ✅ 2.1 SSRF 与 本地文件泄露 (Fixed)
*   **状态**: **已修复** (v2.3.3)
*   **修复方案**:
    - **逻辑拦截**: 在 `src/utils.py` 中引入 `is_safe_url` 函数。
    - **DNS 解析校验**: 在图片下载请求发出前，将主机名解析为 IP 地址。
    - **内网拦截**: 严格禁止访问私有 IP (RFC 1918)、回环地址 (127.0.0.1/::1)、链路本地地址及保留地址。
    - **协议限制**: 仅允许 `http` 和 `https` 协议。
*   **收益**: 有效防止了利用推文图片链接探测内网接口或读取本地文件的风险。
